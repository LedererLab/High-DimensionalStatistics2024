...
3.1 Importing and Scaling a Real Data Set
```{r}
...
data=data.original[,6:128]  #deleting non-predictive features
n=dim(data)[1]    #saving dimensions
p=dim(data)[2]
data=as.numeric(as.matrix(data, nrow=n,ncol=p))   #converting to numeric values
data=matrix(data,nrow=n,ncol=p)                   #converting the vector from as.numeric back to a matrix
data=na.omit(data)
#scaling
#data=scale(data/sqrt(dim(data)[1]-1)*sqrt(dim(data)[1]))
design=data[,-dim(data)[2]]
